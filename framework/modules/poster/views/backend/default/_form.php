...
<div class="poster-form">
    ...
    <fieldset>
        <section>
			<?= $form->field($model, 'name', ['template' => '{label} <label class="input"><i class="icon-prepend fa fa-television"></i>{input}{error}{hint}</label>'])->textInput(['maxlength' => true, 'placeholder' => 'Название афиши']) ?>
        </section>
        <section>
			<?= $form->field($model, 'description', ['template' => '{label} <label class="textarea"><i class="icon-prepend fa fa-info"></i>{input}{error}{hint}</label>'])->textarea(['rows' => 6, 'placeholder' => 'Описание афиши']) ?>
        </section>
        <section>
            <?= $form->field($model, 'category_id')->dropDownList(Filters::ListOfFilters(Filters::CATEGORY_POSTER)) ?>
        </section>
        <section>
			<?= $form->field($model, 'published')->dropDownList(Poster::PublishedStatus()) ?>
        </section>
        <section>
        ...
        <div class="row">
            ...
            <section class="col col-6 text-center">
				<? if ($model->isNewRecord || empty($model->image)):
					...
					echo $form->field($model, 'image', ['template' => '{label} <label class="input" id="images"><a href="" id="thumb-image0" data-toggle="image"><img style="width: 80%;"   src="' . \Yii::$app->imageresize->getUrl($model->image, 320, 180, 'outbound', 100,
        ...
        <section>
            ...
            <div class="inline-group">
                <label class="checkbox">
                    <?= Html::Checkbox('Poster[pin_main]', $model->pin_main,[ 'checked' => $model->pin_main])?>
                    <i></i>
                    На главной
                </label>
                <label class="checkbox">
                    <?= Html::Checkbox('Poster[pin_poster]',$model->pin_poster)?>
                    <i></i>
                    В афише на главной
                </label>
                <label class="checkbox">
                    <?= Html::Checkbox('Poster[pin_filter]',$model->pin_filter)?>
                    <i></i>
                    В фильтре
                </label>
            </div>
