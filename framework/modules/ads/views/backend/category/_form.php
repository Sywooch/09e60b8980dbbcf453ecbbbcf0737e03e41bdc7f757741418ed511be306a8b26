...
/* @var $model app\modules\ads\models\Category */
    ...
    <fieldset>
        <div class="row">
            <section class="col col-sm-12 col-lg-12">
                    ...
                    <section><?= $form->field($model, 'parent_id')->dropDownList(ArrayHelper::merge(['id' => null,'title'=>'Родительская категория'], Category::ListOfCategories())) ?></section>
                    ...
                    <section>
                        <? if($model->isNewRecord || empty($model->image)):
	                            ...
	                            echo $form->field($model, 'image', ['template' => '{label} <label class="input" id="images"><a href="" id="thumb-image0" data-toggle="image"><img style="width: 80%;"   src="'.\Yii::$app->imageresize->getUrl(Yii::getAlias('@webroot' . $model->image), 100, 100, 'outbound', 0, Yii::getAlias('@webroot/uploads/cache/' . $model->image)) .'" alt="" title="" data-placeholder="'.$placeholder.'" />{input}{error}{hint}</label></a>'])->hiddenInput();
                            endif;
                        ?>
                    </section>
            </section>
