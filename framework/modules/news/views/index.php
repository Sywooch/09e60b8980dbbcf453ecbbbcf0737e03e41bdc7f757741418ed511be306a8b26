...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-8 text-align-right">
        <div class="page-title">
            <?= Html::a('Опубликованные', Url::toRoute('index'), ['class' => 'btn btn-default' . ($type == 'published' ? ' disabled' : ''), 'onclick' => $type == 'published' ? 'return false;' : '']) ?>
            <?= Html::a('Скрытые', Url::toRoute('index/type/hidden'), ['class' => 'btn btn-default' . ($type == 'hidden' ? ' disabled' : ''), 'onclick' => ($type == 'hidden' ? 'return false;' : '')]) ?>
            <?= Html::a('Входящие', Url::toRoute('index/type/new'), ['class' => 'btn btn-default' . ($type == 'new' ? ' disabled' : ''), 'onclick' => $type == 'new' ? 'return false;' : '']) ?>
            <?= Html::a('Комментарии', Url::toRoute('comments'), ['class' => 'btn btn-default']) ?>
                ...
                <div class="widget-body">
                    <? $form = ActiveForm::begin(['action' => ['index', 'type' => $type], 'method' => 'get', 'options' => ['class' => 'd-block']]); ?>
                        <?= $form->field($searchModel, 'title',
                                ['template' => '<div class="input-group">{input}<div class="input-group-btn"><button class="btn btn-primary" type="submit"><i class="fa fa-search"></i> Поиск</button></div></div>'])
