...
?>
            ...
            <section>
                ...
                if ($model->isNewRecord || (empty($model->image) || !file_exists(Yii::getAlias($model->image)))):
                    ...
                    echo $form->field($model, 'image', ['template' => '{label} <label class="input" id="images"><a href="" id="thumb-image0" data-toggle="image"><img style="width: 80%;"   src="' . \Yii::$app->imageresize->getUrl(Yii::getAlias($model->image), 100, 100, 'outbound', 0, Yii::getAlias('uploads/cache/' . $model->image)) . '" alt="" title="" data-placeholder="' . $placeholder . '" />{input}{error}{hint}</label></a>'])->hiddenInput();
                endif;
                ?>
            </section>
        </section>
