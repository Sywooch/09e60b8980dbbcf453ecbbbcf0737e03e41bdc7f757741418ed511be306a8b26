...
class MAILER {
    ...
    private static function getConfirmContent($email, $hash) {
        ...
        <p>Перейдите пожалуйста по этой ссылке для подтверждения 
            <a href="http://<?= MAIN_DOMAIN ?>/confirm.php?action=mail_confirm&email=<?= $email ?>&hash=<?= $hash ?>">
                http://<?= MAIN_DOMAIN ?>/confirm.php?action=mail_confirm&email=<?= $email ?>&hash=<?= $hash ?>
            </a>
        </p>
        <?
    ...
    public static function sendResetEmail($user) {
        $hash = User::getHash($user);
//        var_dump($hash);die;
        $content = self::getResetContent($user['email'], $hash);
        return self::send($user['email'], $content, 'Сбросить пароль');
    }

    private static function getResetContent($email, $hash) {
        ob_start();
        ?>
        <h2>Восстановить пароль</h2>
        <p>Перейдите пожалуйста по этой ссылке для восстановления 
            <a href="http://<?= MAIN_DOMAIN ?>/confirm.php?action=reset&email=<?= $email ?>&hash=<?= $hash ?>">
                http://<?= MAIN_DOMAIN ?>/confirm.php?action=reset&email=<?= $email ?>&hash=<?= $hash ?>
            </a>
        </p>
        <?
        $Content = ob_get_contents();
