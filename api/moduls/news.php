...
class News {
    ...
    if (!empty($_GET['id'])) {
      if ((int) $_GET['id'] > 0) {
        ...
        if ($item) {
          ...
          $data['item'] = array_merge((array) $data['item'], (array) DB::q_line($query));
          ...
          if (!empty($user_ids)) {
            ...
            $in = implode(',', $user_ids);
            $db_user = new DB('user');
            $query = "SELECT id, name, f_name, photo_250 FROM `user` WHERE id IN(" . $in . ")";
            $data['users'] = $db_user::q_array();
          }
        }
      }
    }
  ...
  private static function commmentUpdate($id) {
    $user = User::get();
    if (!empty($_POST['comment']) && $user && $id) {
